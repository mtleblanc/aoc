 ...
 */
size_t pack(std::span<size_t> vals, size_t target)
{
    // first index: number of items included
    // second index: total size of items included
    // value: smallest product of sizes
    std::vector<std::vector<std::optional<size_t>>> dp;
    std::optional<size_t> smallestGroupFound;
    for (auto v : vals)
    {
        if (v > target)
        {
...
{
    auto boxes = readAll<size_t>(input);
